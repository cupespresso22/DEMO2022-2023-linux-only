...
Самопальная реализация "отказоустойчивости" в виде скрипта. Суть его работы в том, что раз в секунду пингуется RTR-L и RTR-R. В случае недоступности RTR-L, на ISP файл demo.db с данными о A- и CNAME-записях заменяется на аналогичный, но без данных о RTR-L. И наоборот — в случае недоступности RTR-R, из demo.db "убираются" данные о RTR-R. Это делается для того, чтобы DNS-сервер на ISP не отсылал запросы на неработающий маршрутизатор и не ждал от него ответа. Помимо самого скрипта, для работы такого рода "отказоустойчивости"
